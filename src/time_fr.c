...
int french_time(struct tm* t, char* line1, char* line2, char* line3) {
  int ret = 1;
  ...
  if (minutes >= 0 && minutes < 3) {
    ...
    else strcat(line2, MINS[5]); // et demi
  }
  else if (minutes < 38) {
...
void french_info_lines(struct tm* t, char* line1, char* line2) {
...
//  strftime(line2, LINE_BUFFER_SIZE, "s-%W • j-%j • %H%M", t);
  ...
  strftime(line2, LINE_BUFFER_SIZE, "%H:%M - ", t);
  strcat(line2, wday);
  strcat(line2, " ");
  strcat(line2, mday);
  strcat(line2, " ");
